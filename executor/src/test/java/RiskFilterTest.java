...
public class RiskFilterTest {
    ...
    void rejectsWhenEdgeBelowThreshold() {
        RiskFilter filter = new RiskFilter(2.0, 100, 1.0);
    ...
    void rejectsInvalidNetEdge() {
        RiskFilter filter = new RiskFilter(0.0, 100, 1.0);
    ...
    @Test
    void rejectsWhenSlippageTooHigh() {
        RiskFilter filter = new RiskFilter(1.0, 100, 0.5);
        SpreadOpportunity opp = new SpreadOpportunity("BTC/USDT", "A", "B", 2.0, 1.0, 50);
        assertFalse(filter.passes(opp), "High slippage opportunity should be rejected");
    }
}
